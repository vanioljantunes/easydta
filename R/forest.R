...
#'
#' @param columns Optional data frame of extra per-study columns drawn
#'   between the study label and the 2x2 counts (for example method, test
#'   type and cutoff). One row per study, matched by a `studlab` column when
#'   present, otherwise in the order of the studies in `fit`. Column names
#'   are the headers. A column named `Study` replaces the displayed label.
#' @param overall Logical. `FALSE` drops the overall pooled row, its
#'   reference line and the I-squared row, for forests whose subgroups
#'   should not be pooled together. Default `TRUE`.
#'
...
dta_forest <- function(fit, test, conf = 0.95, digits = 2,
  ...
  label_df <- data.frame(ypos = rows$ypos, studlab = rows$label,
                         ...
                         face = face, type = rows$type,
                         stringsAsFactors = FALSE)

  # extra per-study columns, blank on heading and summary rows
  extra_cols <- list()
  if (!is.null(columns)) {
    columns <- as.data.frame(columns, stringsAsFactors = FALSE)
    ci <- if ("studlab" %in% names(columns))
      match(studs, as.character(columns$studlab)) else seq_len(nrow_d)
    if (anyNA(ci) || length(ci) != nrow_d)
      stop("`columns` must have one row per study in `fit`.")
    columns <- columns[ci, setdiff(names(columns), "studlab"), drop = FALSE]
    si <- match(rows$label, studs)
    si[rows$type != "study"] <- NA
    for (nm in names(columns)) {
      v <- ifelse(is.na(si), "", as.character(columns[[nm]][si]))
      v[is.na(v)] <- ""
      if (nm == "Study") {
        label_df$studlab <- ifelse(is.na(si), label_df$studlab, v)
      } else {
        extra_cols[[nm]] <- v
      }
    }
  ...
  i2_y      <- 0
  test_y    <- -1
  ylim_full <- c(if (!is.null(sub_p)) -1.5 else if (isTRUE(overall)) -0.5 else 0.5,
                 header_y + 0.5)
  ...
  # ----- Label panel: Study, optional extra columns, then TP FN TN FP --------
  # Column positions come from character widths so extra columns never
  # overlap; text columns are left-aligned, the counts right-aligned.
  hdr_size  <- 3.2
  cell_size <- 3.0
  lab_cols <- c(list(Study = label_df$studlab), extra_cols,
                list(TP = label_df$TP, FN = label_df$FN,
                     TN = label_df$TN, FP = label_df$FP))
  lab_right <- names(lab_cols) %in% c("TP", "FN", "TN", "FP")
  # subgroup headings may run over the blank cells to their right, so they
  # do not set the Study column width
  lab_w <- vapply(seq_along(lab_cols), function(i)
    max(nchar(c(names(lab_cols)[i], lab_cols[[i]][label_df$type != "group"])),
        na.rm = TRUE) + if (lab_right[i]) 1.5 else 2.5, numeric(1))
  lab_edge  <- cumsum(lab_w) / sum(lab_w)
  lab_start <- c(0, head(lab_edge, -1))
  lab_x     <- ifelse(lab_right, lab_edge, lab_start)
  cells <- do.call(rbind, lapply(seq_along(lab_cols), function(i) data.frame(
    x = lab_x[i], ypos = label_df$ypos, label = lab_cols[[i]],
    face = label_df$face, hjust = if (lab_right[i]) 1 else 0,
    stringsAsFactors = FALSE)))
  cells <- cells[!is.na(cells$label) & cells$label != "", , drop = FALSE]
  heads <- data.frame(x = lab_x, ypos = header_y, label = names(lab_cols),
                      hjust = ifelse(lab_right, 1, 0), stringsAsFactors = FALSE)
  label_w_in <- max(3.0, 3.0 * sum(lab_w) / 43)

  p_labels <- ggplot2::ggplot(cells, ggplot2::aes(y = ypos)) +
    zebra_layer() +
    ggplot2::geom_text(ggplot2::aes(x = x, label = label, fontface = face,
                                    hjust = hjust), size = cell_size) +
    ggplot2::geom_text(data = heads,
                       ggplot2::aes(x = x, label = label, hjust = hjust),
                       fontface = "bold", size = hdr_size) +
    (if (isTRUE(overall))
      ggplot2::annotate("text", x = 0, y = i2_y,
                        label = i2_biv_text, hjust = 0,
                        size = i2_size)) +
    (if (!is.null(sub_p))
      ggplot2::annotate("text", x = 0, y = test_y,
  ...
  main <- gridExtra::arrangeGrob(
    ...
    ncol = 5,
    widths = c(label_w_in, 1.8, 1.6, 1.8, 1.6),
